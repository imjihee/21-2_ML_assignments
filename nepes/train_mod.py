...
def train(epoch, config, model, optimizer, scheduler, loss_func, train_loader,
    ...
    for step, (data, targets) in enumerate(train_loader):
        ...
        data_chunks, target_chunks = subdivide_batch(config, data, targets) # subdivision : 1
        optimizer.zero_grad()
        outputs = []
        losses = []
        ...
        if config.dataset.n_classes == 2:
            acc1 = compute_accuracy(config,
                                    outputs,
                                    targets,
                                    augmentation=True,
                                    topk=(1,))
...
def main():
    ...
    train_dataset, val_dataset = create_dataset(config, is_train=True)
    train_loader, val_loader = create_dataloader(config, train_dataset, val_dataset, is_train=True)
